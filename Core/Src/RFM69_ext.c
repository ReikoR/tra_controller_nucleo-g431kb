...
{
HAL_GPIO_WritePin(RFM69_NSS_GPIO_Port, RFM69_NSS_Pin, SetCSValue); //GPIOA, GPIO_PIN_5 is LED on Nucleao, for testing only.
}

bool RFM69_ReadDIO0Pin(void)       // function to read GPIO connected to RFM69 DIO0 (RFM69 interrupt signalling)
{
  if (GO_AHEAD_READ_RFM69 == 1 && HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_5) == 1)
  {
    return true;
  }
  else
